...
#base_time = pd.to_datetime('2025-04-02 09:35:10')
agents.extend([ImpactAgent(id=j,
                             name="IMPACT_AGENT_{}".format(j),
                             type="ImpactAgent",
                             symbol=symbol,
                             starting_cash=starting_cash,
                             greed = 0.90,
                             within = 0.1,
                             impact = True, 
                            impact_time = pd.to_datetime('2025-04-03 13:56:10'),
                             random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                       dtype='uint64')))
               for j in range(agent_count, agent_count + num_momentum_agents)])
agent_count += num_momentum_agents
agent_types.extend("ImpactAgent")
