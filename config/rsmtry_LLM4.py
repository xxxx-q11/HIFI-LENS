...
from agent.follow_tradeAgent import follow_tradeAgent
...
#6) Momentum Agents
num_follow_tradeAgent_agents = 200

agents.extend([follow_tradeAgent(id=j,
                             name="follow_tradeAgent{}".format(j),
                             type="follow_tradeAgent",
                             symbol=symbol,
                             starting_cash=starting_cash,
                             min_size=1,
                             max_size=10,
                             ...
                             random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                       dtype='uint64')))
               for j in range(agent_count, agent_count + num_follow_tradeAgent_agents)])
agent_count += num_follow_tradeAgent_agents
agent_types.extend("follow_tradeAgent")
