...
class Order:
    order_id = 0        # The currently allocatable order ID counter
    _order_ids = set()  # The existing order ID set (ensures uniqueness)
    ...
    def __init__(self, agent_id, time_placed, symbol, quantity, is_buy_order, order_id=None, tag=None):
        ...
        # Equity symbol for the order. Stock code
        self.symbol = symbol

        # Number of equity units affected by the order. Order quantity
        self.quantity = quantity

        # Boolean: True indicates a buy order; False indicates a sell order. Buy or sell
    ...
    def generateOrderId(self):
        # Generate a globally unique order ID (recursively resolves conflicts, ensuring thread safety)
        # generates a unique order ID if the order ID is not specified
        # Use current Order.order_id when it is not occupied
