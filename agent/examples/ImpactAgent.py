...
class ImpactAgent(TradingAgent):
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    if self.state == 'AWAITING_SPREAD':
      ...
      if msg.body['msg'] == 'QUERY_SPREAD':
        ...
        print(f"ask_liq: {ask_liq}, bid_liq: {bid_liq}")
        ...
        if self.impact: 
          ...
          self.placeLimitOrder (self.symbol, shares, direction, price-1)
        else:
  ...
  def computeRequiredPrice(self, direction, shares):
    """
    Calculate the limit price required to immediately execute a specified quantity of stocks.

    direction: True indicates a buy order, and False indicates a sell order

    shares: The number of stocks that need to be traded

    Return: The price required to meet the transaction quantity
    """
    # When placing a buy order, check the sell order (asks); when placing a sell order, check the buy order (bids)
    book = self.known_asks[self.symbol] if direction else self.known_bids[self.symbol]

    t = 0  # The cumulative number of stocks that can be traded

    for p, v in book:
      t += v
      # If the cumulative number of stocks reaches the target, return the current price
      if t >= shares:
        return p

    # If the order book depth is not enough, return the worst price (High sell price or Low buy price)
    return book[-1][0]
