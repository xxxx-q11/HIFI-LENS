...
class ValueAgent(TradingAgent):
    ...
    def kernelStopping(self):
        ...
        print("Final relative surplus", self.name, surplus)  # 计算相对收益
        print(self.holdings)
    ...
    def wakeup(self, currentTime):
        ...
        next_wakeup_time = currentTime + pd.Timedelta('{}ns'.format(int(round(delta_time))))
        self.setWakeup(next_wakeup_time)
        # if next_wakeup_time>self.mkt_close and (not self.symbol in self.daily_close_price):
    ...
    def updateEstimates(self):
        ...
        obs_t = self.oracle.observePrice(self.symbol, self.currentTime, self.id,
                                         random_state=self.random_state)
        # self.r_bar = self.oracle.observePrice(self.symbol, self.currentTime, self.id, sigma_n = 0,
        #                                  random_state=self.random_state)
