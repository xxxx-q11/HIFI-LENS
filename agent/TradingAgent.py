...
class TradingAgent(FinancialAgent):
  ...
  # Send a request to the ExchangeAgent to get market news
  def requestMarketNews(self, symbol):
    self.sendMessage(recipientID=self.exchangeID,
                     msg=Message({"msg": "MARKET_NEWS_REQUEST",
                                  "sender": self.id, "symbol": symbol}))
  ...
  def queryLastTrade (self, symbol, price):
    ...
    #print("Received last trade price of {} for {}.", self.last_trade[symbol], symbol)
    # time.sleep(10)
  ...
  def getKnowBiDAsK_all(self, symbol):
    # bids = self.known_bids[symbol] if self.known_bids[symbol] else None
    # asks = self.known_asks[symbol] if self.known_asks[symbol] else None
    bids = self.known_bids.get(symbol)
    asks = self.known_asks.get(symbol)
    last_trade = self.last_trade[symbol] if self.last_trade[symbol] else None
