...
from util.News import News_before_open,Policy_before_open
from run import parallel_process_opening_price,parallel_chose_agent_strategy
import time
import json
...
class Kernel:
  ...
  def runner(self, agents = [], startTime = None, stopTime = None,
             num_simulations = 1, defaultComputationDelay = 1,symbol = 'AAPL',
             defaultLatency = 1, agentLatency = None, latencyNoise = [ 1.0 ],
             agentLatencyModel = None, skip_log = False,trading_days = None,
             seed = None, oracle = None, log_dir = None,num_example = None):
    ...
    for sim in range(num_simulations):
      ...
      # Initialize the real-time visualization module and pass in the ExchangeAgent instance
      print ("\n--- Initialize the real-time visualization module ---")
      real_time_visualization.initialize(self.agents[0])
      ...
      while self.currentTime:
        print("Day{}".format(current_day_index))
        if current_day_index >= 1:
          ...
          self.startTime = pd.to_datetime(self.trading_days[current_day_index]) + pd.to_timedelta('00:00:00')
          self.stopTime = pd.to_datetime(self.trading_days[current_day_index]) + pd.to_timedelta('16:01:00')
          mkt_open = pd.to_datetime(self.trading_days[current_day_index]) + pd.to_timedelta('09:30:00')
          mkt_close = pd.to_datetime(self.trading_days[current_day_index]) + pd.to_timedelta('16:00:00')
          self.agents[0].mkt_open = mkt_open
          self.agents[0].mkt_close = mkt_close
          self.agents[0].last_day_trade = self.agents[0].update_last_day_trade_history()
          self.agents[0].last_day_trade_price_len = int(self.agents[0].return_len_trade_price())
          self.oracle.mkt_open = mkt_open
          self.oracle.mkt_close = mkt_close
          self.messages = queue.PriorityQueue() # Clear all queue information from yesterday
          self.agents[0].cancel_all_open_orders() # Cancel the orders in the order book
          for agent in self.agents:
            agent.kernelStarting(self.startTime)
          self.agentCurrentTimes = [self.startTime] * len(agents)
          # Set the kernel to its startTime.
        ...
        self.initialize_trading_day(current_day_index,symbol)        # Update the opening price
          # Track starting wall clock time and total message count for stats at the end.
        ...
        current_day_index += 1
        if current_day_index >= len(self.trading_days):
          break

        
        #current_day_index += 1
      with open(f"Experimental_data/result_data/json_Krenel_LLM_data.json", "w", encoding="utf-8") as f:
            json.dump({str(k): v for k, v in self.LLM_data.items()}, f, ensure_ascii=False, indent=2)
  ...
  def msg_before_open(self,date):
    json_message = {
      "News": News_before_open[date],  # News information is already in dictionary format
      "policy":Policy_before_open[date],
      "date": date + '09:29:00',
      "last_day_trade": self.agents[0].last_day_trade
      }
  ...
  def initialize_trading_day(self, current_day_index, symbol):
      """
      Initialize the trading day, process the LLM agent, opening price and trading strategy
      """
      # Get the LLM agent
      agent_LLM = [llm for agent in self.agents[1:1+self.num_example] for llm in agent.LLM_agent]
      print(len(agent_LLM))
      
      # Process the message before opening
      message_before_opening = self.msg_before_open(str(self.startTime).split()[0])
      print(self.startTime)
      print(message_before_opening)
      
      # Check the cache (modified part - use Ray Agent state management)
      cache = get_cache()
      if cache and cache.cache_mode:
          cached_opening_results = cache.get_cached_opening_results_with_state()
          if cached_opening_results:
              open_price, agent_strategy_list, agent_state = cached_opening_results
              print(f"Use the cached opening LLM results, skip the actual call")
              
              # Restore the Agent state of the last LLM call at the opening
              if agent_state:
                  deserialize_agents_state(agent_LLM, agent_state)
                  ...
                  print(f"Restore the Agent state of the last LLM call at the opening: {summary}")
                  print("The Agent has resumed running from where it was interrupted last time")
          else:
              print("The opening cache has been used up, switch to the actual LLM call")
              open_price = parallel_process_opening_price(agent_LLM, message_before_opening)
              agent_strategy_list = parallel_chose_agent_strategy(agent_LLM)
              
              # Save to the cache (including Ray Agent state)
              if cache and not cache.cache_mode:
                  agent_state = serialize_agents_state(agent_LLM)
                  cache.add_opening_results_with_state(open_price, agent_strategy_list, agent_state)
      else:
          # Normal call LLM
          open_price = parallel_process_opening_price(agent_LLM, message_before_opening)
          agent_strategy_list = parallel_chose_agent_strategy(agent_LLM)
      ...
      self.LLM_data.update({f"{current_day_index}open_price": open_price})
      
      # Calculate the average opening price
      open_price_mean = int(sum(open_price) / len(open_price))
      
      # Update price data
      for i in range(len(self.oracle.llm_changed_price)):
          for j in range(len(self.oracle.llm_changed_price[i])):
              self.oracle.llm_changed_price[i][j] = open_price_mean
      
      # Update LLM data
      self.LLM_data.update({f"{current_day_index}oracle_open_price": open_price_mean})
      self.LLM_data.update({f"{current_day_index}agent_strategy_list": agent_strategy_list})
      
      print(self.oracle.llm_changed_price)
      print(agent_strategy_list)
      
      # Determine the strategy for each agent
      same_LLM_type_parallel_num = len(agent_strategy_list) // self.num_example
      agent_strategy_2d = [agent_strategy_list[i*same_LLM_type_parallel_num:(i+1)*same_LLM_type_parallel_num] for i in range(self.num_example)]
      self.LLM_data.update({f"{current_day_index}agent_strategy_2d": agent_strategy_2d})
      
      # Assign policies to each agent
      for agent in self.agents[1:1+self.num_example]:
          agent.strategy_chose(agent_strategy_2d[agent.id-1])
      
      # Set the final transaction price
      self.agents[0].order_books[symbol].last_trade = open_price_mean
